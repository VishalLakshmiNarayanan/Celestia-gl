...
export function GlobeComponent({
  ...
  const globeReadySignal = useRef(false) // set by onGlobeReady, committed after mount
  const [globeReady, setGlobeReady] = useState(false)

  // Commit early "ready" only after mount
  useEffect(() => {
    isMounted.current = true
    if (globeReadySignal.current && !globeReady) setGlobeReady(true)
    return () => {
      isMounted.current = false
    }
  }, [globeReady])

  // Initialize camera + autorotate once ready (with proper cleanup)
  useEffect(() => {
  ...
  return (
    ...
    <div className="relative w-full h-[min(75vh,720px)]">
      <Globe
        ...
        // Animation + readiness (avoid built-in tween race)
        animateIn={false}
